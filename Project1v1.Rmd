...
We can also use the pairwise t-test, included in the rstatix package, which allows for us to compare more than two categorical variables.
...
Now that we have some statistical analysis of our data, as well as some figures to better visualize it, the next step is to combine the two to allow for an easy interpretation. For the sake of this example, let's add in our results from the previously preformed pairwise t-test on diet and body mass. Using the package ggpubr, we can combine statistical tests with our graphs to create some publication-quality figures. 
