...
```{r mutate}
palmerpenguins_extended <- mutate(palmerpenguins_extended, BCI = body_mass_g / flipper_length_mm)
summary(palmerpenguins_extended$BCI)
```
...
```{r filter}
palmerpenguins_extended.filter <- filter(palmerpenguins_extended, body_mass_g < 6038)
dim(palmerpenguins_extended.filter)
```
...
```{r histogram}
ggplot(palmerpenguins_extended) +
  aes(x=body_mass_g) +
